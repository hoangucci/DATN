...
namespace Game.Editor
{
    ...
    {
        [MenuItem("Game Utility/Build Muck-Style UI (Modal Settings Overlay & Controls)")]
        public static void BuildMuckUI()
        {
            ...
            GameObject logoObj = GameObject.Find("logo game") ?? GameObject.Find("Logo Game") ?? GameObject.Find("Logo");
            ...
            // 2. Tạo / Cập nhật Lớp Phủ Tối Mờ Toàn Màn Hình (Settings Modal Overlay / Dark Backdrop)
            ...
            overlayImg.color = new Color(0f, 0f, 0f, 0.75f); // Tối mờ 75% khoá toàn bộ màn hình đằng sau
            overlayImg.raycastTarget = true;

            // Ban đầu ẩn Overlay
            overlayObj.SetActive(false);

            // Gán biến settingsOverlayBackdrop trong MainMenuManager
            if (mainManager != null)
            {
                SerializedObject mainSO = new SerializedObject(mainManager);
            ...
            // Click ra ngoài tối mờ để đóng Settings
            ...
            // 3. Kiểm tra: KHÔNG tạo lại nếu 2 khung Lobby Members và Lobby Join đã tồn tại trong Scene
            GameObject existingMembersPanel = GameObject.Find("[Muck_LobbyMembersPanel]");
            if (existingMembersPanel == null)
            {
                ...
                CreateDummyMemberItem(listContainer.transform, "Snakester95 [Host]");
                CreateDummyMemberItem(listContainer.transform, "Teammate_02");
                CreateDummyMemberItem(listContainer.transform, "Teammate_03");
            ...
            GameObject existingJoinPanel = GameObject.Find("[Muck_LobbyJoinPanel]");
            if (existingJoinPanel == null)
            {
                GameObject joinPanel = CreateOrResetPanel(canvas.transform, "[Muck_LobbyJoinPanel]",
                    new Vector2(1f, 0f), new Vector2(1f, 0f), new Vector2(-30f, 30f), new Vector2(320f, 220f));

                CreateLabel(joinPanel.transform, "TitleLabel", "LOBBY ID: (send to friend)", 16, TextAlignmentOptions.Center,
                    new Vector2(0.5f, 1f), new Vector2(0.5f, 1f), new Vector2(0f, -20f), new Vector2(280f, 30f));

                CreateInputField(joinPanel.transform, "IpInputField", "Click to reveal / Type Lobby Code...",
                    new Vector2(0.5f, 1f), new Vector2(0.5f, 1f), new Vector2(0f, -55f), new Vector2(280f, 40f));

                CreateMuckButton(joinPanel.transform, "CopyIpButton", "Copy to clipboard",
                    new Vector2(0.5f, 0f), new Vector2(0.5f, 0f), new Vector2(0f, 25f), new Vector2(270f, 38f));
            }

            // 4. Đồng bộ UI hiệu ứng & màu sắc của TẤT CẢ các nút bấm trong Settings Panel
            GameObject settingsPanel = GameObject.Find("SettingsPanel") ?? GameObject.Find("Settings") ?? GameObject.Find("settingsPanel");
            if (settingsPanel != null)
            {
                // Cài đặt Canvas riêng cho Settings Panel nổi lên trên cùng (Sorting Order = 100)
                ...
                // Thêm/Đồng bộ Nút Đăng Xuất (Logout)
                ...
                // Thêm/Đồng bộ Nút Thoát Game (Quit)
                ...
                // ĐỒNG BỘ HIỆU ỨNG & STYLING CHO TẤT CẢ NÚT BẤM BÊN TRONG SETTINGS
                SynchronizeSettingsButtonsStyle(settingsPanel);
            }

            EditorSceneManager.MarkSceneDirty(canvas.gameObject.scene);
            EditorUtility.DisplayDialog("Hoàn tất!",
                "Đã hoàn thành 2 yêu cầu của bạn:\n\n" +
                "1. Tự động kiểm tra giữ nguyên khung Lobby cũ của bạn (KHÔNG tạo đè hay tạo lại nữa).\n" +
                "2. Đồng bộ 100% hiệu ứng UI, viền gỗ, hiệu ứng rê chuột & click cho TẤT CẢ các nút bấm trong phần Settings!", "OK");
        }
        ...
        private static void SynchronizeSettingsButtonsStyle(GameObject settingsPanel)
        {
            ...
            Color woodNormal = new Color(0.48f, 0.3f, 0.16f, 1f);       // Nâu gỗ chuẩn Muck
            Color woodHover = new Color(0.65f, 0.42f, 0.24f, 1f);      // Nâu sáng rực rỡ khi Rê chuột (Hover)
            Color woodPressed = new Color(0.32f, 0.19f, 0.09f, 1f);    // Nâu sẫm khi Bấm xuống (Click)
            Color woodDisabled = new Color(0.25f, 0.2f, 0.18f, 0.6f);

            foreach (Button btn in allButtons)
            {
                // Set Transition Color Block
                ...
                // Synchronize Font & Text styling
                TMP_Text tmpText = btn.GetComponentInChildren<TMP_Text>();
                if (tmpText != null)
                {
                    tmpText.fontStyle = FontStyles.Bold;
                    tmpText.color = new Color(0.98f, 0.92f, 0.78f); // Vàng kem gỗ rực rỡ
